/* NVS-backed SoftAP and APN configuration helpers. */
...
static const apn_profile_desc_t s_apn_profiles[] = {
    { APP_APN_PROFILE_MOBILE, "China Mobile", "cmnet" },
    { APP_APN_PROFILE_UNICOM, "China Unicom", "3gnet" },
    { APP_APN_PROFILE_TELECOM, "China Telecom", "ctnet" },
    { APP_APN_PROFILE_CUSTOM, "Custom", "" },
};
...
void app_config_set_defaults(app_config_t *config)
{
    ...
    config->apn_profile = APP_APN_PROFILE_TELECOM;
}
...
esp_err_t app_config_validate(const app_config_t *config, const char **error_message)
{
    ...
    if (config == NULL) {
        if (error_message != NULL) {
            *error_message = "Config object is null.";
        }
    ...
    if (ssid_len == 0 || ssid_len > 32) {
        if (error_message != NULL) {
            *error_message = "Wi-Fi SSID length must be 1 to 32 characters.";
        }
        return ESP_ERR_INVALID_ARG;
    }
    ...
    if (password_len < 8 || password_len > 63) {
        if (error_message != NULL) {
            *error_message = "Wi-Fi password length must be 8 to 63 characters.";
        }
        return ESP_ERR_INVALID_ARG;
    }
    ...
    if (!app_config_wifi_channel_is_valid(config->channel)) {
        if (error_message != NULL) {
            *error_message = "Wi-Fi channel must be 1, 6, or 11.";
        }
        return ESP_ERR_INVALID_ARG;
    }
    ...
    if (config->apn_profile == APP_APN_PROFILE_CUSTOM && config->custom_apn[0] == '\0') {
        if (error_message != NULL) {
            *error_message = "Custom APN cannot be empty.";
        }
        return ESP_ERR_INVALID_ARG;
    }
    ...
    if (strlen(config->custom_apn) >= sizeof(config->custom_apn)) {
        if (error_message != NULL) {
            *error_message = "Custom APN is too long.";
        }
        return ESP_ERR_INVALID_ARG;
    }
...
esp_err_t app_config_load(app_config_t *config)
{
    ...
    uint8_t stored_profile = APP_APN_PROFILE_TELECOM;
    esp_err_t err;
...
const char *app_config_get_apn_profile_label(app_apn_profile_t profile)
{
    ...
    return desc != NULL ? desc->label : "Unknown";
}
...
const char *app_config_get_effective_apn(const app_config_t *config)
{
    ...
    return desc != NULL ? desc->apn : "ctnet";
}
